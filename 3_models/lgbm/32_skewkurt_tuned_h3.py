"""
32_skewkurt_tuned_h3.py

LightGBM GBDT h=3 with v2 features (384) but tighter regularization:
- lambda_l2: 2.87 -> 8.0  (stronger L2)
- min_data_in_leaf: 82 -> 200  (larger leaves, less overfitting)
- feature_fraction: 0.6157 -> 0.50  (model sees fewer features per tree)
- Only top 10 skew/kurt features (drop bottom 10 IC — weaker signal)

Previous result (loose): val=0.1216, LB=0.2400 (overfit)
Baseline (no skew/kurt):  val=0.1103, LB=0.2438
"""
...
# Only keep top 10 IC features' skew/kurt (drop bottom 10)
TOP10_FEATURES = [
...
LGBM_PARAMS = {
    ...
    'max_depth':        7,
    'num_leaves':       88,
    'feature_fraction': 0.50,     # was 0.6157
    'bagging_fraction': 0.8048,
    'bagging_freq':     5,
    'lambda_l1':        0.1182,
    'lambda_l2':        8.0,      # was 2.8783
    'min_data_in_leaf': 200,      # was 82
    'verbose':          -1,
    'n_jobs':           -1,
}
EARLY_STOPPING = 100
...
print(f"  Loose skew/kurt  : 0.1216  ({score_avg - 0.1216:+.4f})")
print(f"  Baseline LGBM    : 0.1103  ({score_avg - 0.1103:+.4f})")
