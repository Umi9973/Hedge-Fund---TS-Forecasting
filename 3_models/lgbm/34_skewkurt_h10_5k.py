"""
34_skewkurt_h10_5k.py

LightGBM GBDT h=10 with v2 features + n_estimators=5000.
Previous run hit 3000-tree cap (seed=2024 stopped at exactly 3000) — model
was still improving, so this gives it room to converge properly.

Previous result (3000 cap): val=0.2245
Baseline LGBM (344 feat):   val=0.2300 (WF Fold4, optimistic)
"""
...
LGBM_PARAMS = {
    ...
    'metric':           'rmse',
    'n_estimators':     5000,     # was 3000 — was hitting cap
...
print(f"\n{'='*60}")
print(f"LightGBM GBDT h={H} | v2 features | 5000 trees | {len(SEEDS)} seeds")
print(f"{'='*60}")
...
print(f"\n{'='*60}")
print(f"RESULTS — h={H} LGBM 5000 trees + v2 features")
print(f"{'='*60}")
print(f"  Per-seed scores  : {[f'{s:.4f}' for s in val_scores]}")
print(f"  Ensemble val     : {score_avg:.4f}")
print(f"  Previous (3000)  : 0.2245  ({score_avg - 0.2245:+.4f})")

val_out           = va[['id', 'ts_index', 'y_target', 'weight']].copy()
val_out['y_pred'] = val_pred_avg
val_out.to_csv(OUT_DIR / f"val_preds_h{H}_skewkurt_5k_cv{score_avg:.4f}.csv", index=False)

test_out               = test_h[['id']].copy()
test_out['prediction'] = test_pred_avg
test_out.to_csv(OUT_DIR / f"test_preds_h{H}_skewkurt_5k_cv{score_avg:.4f}.csv", index=False)
