...
using standard LightGBM GBDT with Optuna-tuned params.

Baselines:
  LightGBM GBDT (344 features): val=0.1103
  CatBoost (344 features):      val=0.1143
  LightGBM DART (344 features): val=0.1034
"""
...
# Same Optuna-tuned GBDT params as baseline (04_model.py h=3)
LGBM_PARAMS = {
    'boosting_type':    'gbdt',
...
print(f"{'='*60}")
print(f"  lr={LGBM_PARAMS['learning_rate']} | depth={LGBM_PARAMS['max_depth']} | leaves={LGBM_PARAMS['num_leaves']}")
...
print(f"  CatBoost        : 0.1143  ({score_avg - 0.1143:+.4f})")

# ── Save ──────────────────────────────────────────────────────
val_out              = va[['id', 'ts_index', 'y_target', 'weight']].copy()
val_out['y_pred']    = val_pred_avg
val_path = OUT_DIR / f"val_preds_h{H}_skewkurt_cv{score_avg:.4f}.csv"
val_out.to_csv(val_path, index=False)

test_out               = test_h[['id']].copy()
test_out['prediction'] = test_pred_avg
test_path = OUT_DIR / f"test_preds_h{H}_skewkurt_cv{score_avg:.4f}.csv"
test_out.to_csv(test_path, index=False)

print(f"\n  Val  preds: {val_path.name}")
print(f"  Test preds: {test_path.name}")
