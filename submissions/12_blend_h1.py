"""
Step 12: Blend LightGBM + Autoencoder NN for h=1
- Retrain h=1 LightGBM (3-seed ensemble) with Optuna params
...
HORIZON_PARAMS = {
    ...
    'bagging_fraction': 0.309, 'early_stopping': 295,
}
...
# TRAIN LGBM (3 seeds)
# ============================================================
print(f"\nTraining LightGBM h={H} (1 seed — seeds 2024/777 stop too early due to low bagging_fraction)...")
...
nn_val_file = AE_DIR / f"val_preds_h{H}_nn_cv0.0501.csv"
...
# Test preds — recover from saved blend + lgbm files (blend = (1-a)*lgbm + a*nn → nn = (blend - (1-a)*lgbm) / a)
PREV_ALPHA    = 0.75
blend_test_df = pd.read_csv(AE_DIR / f"test_preds_h{H}_blend_a{PREV_ALPHA:.2f}_cv0.0591.csv")
lgbm_test_df  = pd.read_csv(AE_DIR / f"test_preds_h{H}_lgbm_cv0.0000.csv")
...
# SAVE
# ============================================================
# Val predictions (lgbm, nn, best blend) — for diagnostics / future use
